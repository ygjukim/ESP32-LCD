...
/**
 * @file GT911_touch_driver.cpp
 * @brief Source file for GT911 touch driver library.
 */
...
/* Functions -----------------------------------------------------------------*/
GT911TouchDriver::GT911TouchDriver(
    TwoWire& wire, uint8_t intPin, uint8_t rstPin, uint8_t addr, uint8_t sdaPin, uint8_t sclPin)
: _wire{wire}, _intPin{intPin}, _rstPin{rstPin}, _addr{addr}, _sdaPin{sdaPin}, _sclPin{sclPin}
{ }
...
void GT911TouchDriver::end() 
{ }

uint8_t GT911TouchDriver::getTouchPoints(GDTpoint_t* points) {
...
void GT911TouchDriver::onDetect(void (*handler)(uint8_t, GDTpoint_t*)) {
...
uint8_t GT911TouchDriver::_gt911WriteOp(uint16_t reg, uint8_t data) {
    uint8_t status = 0;
...
uint8_t GT911TouchDriver::_gt911WriteBytesOp(uint16_t reg, uint8_t * data, uint8_t len) {
    uint8_t status = 0;
...
uint8_t GT911TouchDriver::_gt911ReadOp(uint16_t reg, uint8_t * data, uint8_t len) {
    uint8_t status = 0;
...
void GT911TouchDriver::_gt911onIrq() {
    uint8_t contacts;
    uint8_t rawpoints[GT911_MAX_CONTACTS * GT911_CONTACT_SIZE];
    uint8_t error;
...
uint8_t GT911TouchDriver::_gt911ReadInputCoord(uint8_t * pointsbuf, uint8_t& contacts) {
    uint8_t error;
